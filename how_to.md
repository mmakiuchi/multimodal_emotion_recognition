...
In this part, mel-frequency spectrograms are extracted from the speech files, and they are stored in the directory specified by `spmel_dir`. These spectrograms are later used during the speech emotion recognition model training.

`python make_spect.py --spmel_dir "spmels" --speech_dir "/data/speech"`

### Organize the spectrograms in folds
In this part, the extracted spectrograms are organized in 5 speaker independent folds for a leave-one-session-out cross-validation.
...
For this part, you should have the Gentle files (exp, ext, gentle, align.py, etc) in the aligner folder (check the Gentle repository for more information on how to use the aligner).
...
The dictionary that I used is included in this repository in `aligner/phone_dict.csv`, but you should check if the phones are consistent
